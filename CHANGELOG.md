...
- Add `*exit-fn*`: dynamic var the `:help` option uses to terminate on an *error* (`System/exit` on JVM, `js/process.exit` on Node). `--help`/`-h` is not an error - it prints help and returns, so it does not call this. It receives `{:exit :cause :dispatch :msg :data}` (`:exit` always non-zero; `:cause` is `:missing-subcommand` / `:unknown-subcommand` / the babashka.cli flag cause). Rebind it to avoid exiting (tests/REPL) or to remap exit codes by `:cause`
