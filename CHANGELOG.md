...
- Add a `:help` option to `dispatch`: `(dispatch table args {:help {:prog "mytool"}})` (or `:help true`) wires up help without `:restrict` - it intercepts `--help`/`-h` natively (printing help for the command they follow) and installs `help-error-fn` for bad/missing subcommands and flag errors, reusing dispatch's own table and `:inherit`. `--help`/`-h` are reserved while it is on
