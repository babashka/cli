...
- Add a `:help` option to `dispatch`: `(dispatch table args {:prog "mytool" :help true})` gives a CLI built-in help, no `:restrict` needed. `--help`/`-h` print help for the command they precede and exit with 0 (and `-h, --help` is listed in each command's options); a mistyped or missing subcommand prints help and exits with 1 (a bare group is a usage error, like `git bisect` with no subcommand). Also works for a single-command CLI (one entry with `:cmds []`)
...
- Control the `Options:` order in help with an `:order` (vector of option keys) on a command entry, or on `format-command-help`. It is used verbatim (so you also choose which options to list, and whether to list `--help`). Without it, order follows the spec - a vec-of-pairs spec keeps its order; a map follows key order, which Clojure does not guarantee, so prefer a vec-of-pairs spec or `:order`
