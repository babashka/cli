...
- Add `command`: builds a `dispatch` table entry from a config map, attaching `:cmds` (default `[]`, or a given path). Lets a single-command CLI skip hand-writing `:cmds`: `(dispatch [(command {:fn f :spec spec})] args {:prog "p" :help true})`
