...
- Add a `:help` option to `dispatch`: `(dispatch table args {:prog "mytool" :help true})` gives a CLI built-in help - `--help`/`-h` print help for the command they precede, and a mistyped or missing subcommand prints help too
...
- Add `help-error-fn`: an `:error-fn` for `dispatch` that renders help via `format-command-help` and terminates through `*exit-fn*` (pass it directly: `{:error-fn cli/help-error-fn}`, or via the `:help` option). `--help`/`-h` exits with 0; unknown subcommand, group without subcommand, and flag errors exit with 1 (a bare group is a usage error, like `git bisect` with no subcommand)
