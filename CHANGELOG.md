...
- Option errors now carry `:flag` in their error data (`:restrict` / `:validate` / `:coerce`): the literal option token as typed (e.g. `"--foo"`, `"-f"`, `":foo"`), as opposed to `:option`, the normalized keyword. Lets an `:error-fn` echo what the user actually typed instead of reconstructing it (`-x` and `--x` both parse to `:x`)
...
- Add `*exit-fn*`: dynamic var used by `help-error-fn` to terminate (`System/exit` on JVM, `js/process.exit` on Node). It receives `{:exit :cause :dispatch :message :data}`, where `:cause` is either `:help-requested` / `:missing-subcommand` / `:unknown-subcommand` / the babashka.cli flag cause. Rebind it to avoid exiting (tests/REPL) or to remap exit codes by `:cause`
